...
async function mergeNutritionPanel(glbBuffer, nutrition) {
  ...
  // ── Clean up any panel from a previous run ────────────────────────────────
  // Lets you re-run this tool on an already-enhanced GLB without stacking
  // duplicate panels/materials on top of each other.
  root.listMeshes().forEach(m => { if (m.getName() === 'nutrition-panel') m.dispose(); });
  root.listTextures().forEach(t => { if (t.getName() === 'nutrition-label') t.dispose(); });
  root.listMaterials().forEach(m => { if (m.getName() === 'nutrition-label-material') m.dispose(); });

  // ── De-shine the dish ─────────────────────────────────────────────────────
  // Polycam bakes a metallicRoughness texture that can make parts of the dish
  // reflective. In AR, that catches the environment light and washes the colours
  // out with a grey/white haze. Food isn't metal — force a clean matte material
  // so the real colours come through.
  ...
  const spiceLevel = parseInt(nutrition.spiceLevel, 10) || 0;

  // Nothing to show at all → skip the panel entirely
  if (lines.length === 0 && spiceLevel <= 0) return glbBuffer;

  // If only spice is set, give the renderer a single spacer line so the card
  // has a sensible width for the pepper row.
  if (lines.length === 0 && spiceLevel > 0) lines.push('SPICE LEVEL');

  const { buffer: pngBuffer, width: pxW, height: pxH } = renderTextToPNG(lines, { spiceLevel: spiceLevel });
  ...
  if (minSize === sizeY) {
    ...
    const floatY = maxY + dishFootprint * 0.35 + planeH * 0.6; // float above the food (scaled to plate width, works for flat plates too)
