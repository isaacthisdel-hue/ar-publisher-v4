...
const { LOGO_PNG_BASE64 } = require('./brand');
...
app.get('/brand-logo.png', (req, res) => {
  ...
  res.send(Buffer.from(LOGO_PNG_BASE64, 'base64'));
});
...
:root{
  --bg:#E9E3D7;--panel:#F5F1E8;--panel2:#EFE9DD;
  --ink:#1C1A16;--ink-soft:#6B6355;--ink-faint:#9A9082;
  --bronze:#9A7B4F;--bronze-deep:#7A5F3A;--bronze-glow:rgba(154,123,79,0.14);
  --line:rgba(28,26,22,0.12);--line-strong:rgba(28,26,22,0.22);
  --green:#5C7A4A;--red:#B24A3C;
...
.brand-mark{font-family:var(--disp);font-size:24px;font-weight:700;letter-spacing:.02em;font-style:italic;color:var(--ink)}
...
` : `
<aside class="sidebar">
  <div class="brand"><img src="/brand-logo.png" alt="Servision"></div>
  <div class="brand-tag">AR Publishing Console</div>
  <nav class="nav">
    <button class="nav-item active" id="tab-btn-command" onclick="showTab('command')"><span class="ico">01</span><span class="lbl">Command</span></button>
    ...
    <button class="nav-item" id="tab-btn-restaurants" onclick="showTab('restaurants')"><span class="ico">03</span><span class="lbl">Restaurants</span></button>
  </nav>
...
<main class="workspace">
  ...
  <!-- COMMAND CENTER -->
  <div id="tab-command" class="section active">
    <div class="cc-grid">
      <div class="cc-hero">
...
function showTab(name) {
  ...
  var titles = {
    command: ['Command Center', 'Your daily mission and momentum'],
  ...
  if (name === 'command') refreshCommand();
