...
app.post('/publish', upload.fields([
  ...
  const liveUrl = 'https://' + username + '.github.io/' + repoName + '/' + folderPath;
...
` : `
<div class="tabs">
  <button class="tab active" id="tab-btn-publish" onclick="showTab('publish',this)">📤 Publish Dish</button>
  <button class="tab" id="tab-btn-restaurants" onclick="showTab('restaurants',this)">🏪 Restaurants</button>
</div>
<div class="card">
...
function renderRestaurants(data) {
  ...
  keys.forEach(function(slug) {
    ...
    var branchesHTML = bSlugs.length > 0
      ? bSlugs.map(function(bSlug) {
          return '<span class="branch-tag">' + r.branches[bSlug] +
            '<button onclick="deleteBranch(\'' + slug + '\',\'' + bSlug + '\')" title="Remove branch">×</button></span>';
        }).join('')
      : '<div class="no-branches">No branches yet</div>';

    card.innerHTML =
      '<div class="rest-card-header">' +
        '<div class="rest-name">' + r.name + '</div>' +
        '<button class="btn-danger" onclick="deleteRestaurant(\'' + slug + '\')" title="Delete restaurant">×</button>' +
      '</div>' +
      '<div class="branches-list" id="branches-' + slug + '">' + branchesHTML + '</div>' +
      '<button class="btn-add-branch" onclick="toggleAddBranch(\'' + slug + '\')">+ Add Branch</button>' +
      '<div class="add-branch-form" id="add-branch-form-' + slug + '">' +
        '<input type="text" placeholder="e.g. West Island" id="branch-input-' + slug + '" onkeydown="if(event.key===\'Enter\')saveBranch(\'' + slug + '\')">' +
        '<button class="btn btn-outline" onclick="saveBranch(\'' + slug + '\')">Save</button>' +
        '<button class="btn btn-outline" onclick="toggleAddBranch(\'' + slug + '\')">Cancel</button>' +
      '</div>';
      ...
      .then(function(data) {
        if (data.error) {
          ...
          setStatus('pub-status', 'success',
            ...
            '<button class="copy-btn" onclick="copyText(\'' + data.liveUrl + '\',this)">Copy</button></div>' +
            ...
            '<button class="copy-btn" onclick="copyText(\'' + data.repoUrl + '\',this)">Copy</button></div>' +
