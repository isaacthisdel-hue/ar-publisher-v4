...
const { recordEvent, getStats, analyticsEnabled } = require('./analytics');
...
app.post('/publish', upload.fields([
  ...
  try {
    await pushFile(folderPath + '/index.html', Buffer.from(buildARPage(dishName, brandName, topLabel, logoFileName, theme), 'utf8'));
...
app.post('/track', async (req, res) => {
  ...
  const { dishKey, type, device, viewMs } = req.body || {};
  if (!dishKey || !type) return res.status(400).json({ ok: false });
  await recordEvent(dishKey, type, { device, viewMs });
  res.json({ ok: true });
});
...
// Serve the analytics dashboard HTML for a given dish.
async function serveDashboard(dishKey, res) {
  try {
    const stats = await getStats(dishKey);
...
body{background:
  radial-gradient(1200px 600px at 50% -10%, rgba(200,135,58,0.10), transparent 60%),
  radial-gradient(800px 500px at 100% 100%, rgba(200,135,58,0.05), transparent 55%),
  var(--bg);
...
.card{width:100%;max-width:640px;background:var(--surface);border:1px solid rgba(200,135,58,.3);border-top:none;border-radius:0 0 16px 16px;padding:36px;box-shadow:0 24px 60px -20px rgba(0,0,0,0.55), 0 2px 0 rgba(200,135,58,0.04) inset}
...
function buildARPage(dishName, brandName, topLabel, logoFileName, theme) {
  const THEMES = {
    'dark-elegant': {
