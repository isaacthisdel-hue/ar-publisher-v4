...
app.post('/publish', upload.fields([
  ...
  try {
    await pushFile('index.html', Buffer.from(buildARPage(dishName, brandName, topLabel), 'utf8'));
...
:root{
  --amber:#C8873A;--bg:#111009;--surface:#1A1812;--surface2:#211F17;
  --border:rgba(200,135,58,0.2);--muted:rgba(242,237,228,0.5);--cream:#F2EDE4;--green:#4CAF7D;
}
...
.field input{width:100%;background:var(--surface2);border:1px solid var(--border);border-radius:8px;padding:11px 14px;color:var(--cream);font-family:inherit;font-size:14px;outline:none}
...
.link-box{margin-top:12px;background:var(--surface2);border:1px solid var(--border);border-radius:6px;padding:12px 14px;font-size:12px}
...
<div class="card">
  ...
  <div class="field">
    <label>Company / Brand Name <span style="color:var(--muted);text-transform:none;letter-spacing:0;font-size:10px">(shows at top of AR page)</span></label>
    <input type="text" id="brandName" placeholder="e.g. NOSTRA" maxlength="20">
  </div>
  <div class="field">
    <label>Top Label <span style="color:var(--muted);text-transform:none;letter-spacing:0;font-size:10px">(e.g. "On the menu", "New item")</span></label>
    <input type="text" id="topLabel" placeholder="On the menu" maxlength="30">
  </div>
  ...
  <div class="field">
    <label>AR Model — .usdz file <span style="color:var(--muted);text-transform:none;letter-spacing:0;font-size:10px">(iPhone AR)</span></label>
...
if (btn) {
  btn.addEventListener('click', function() {
    ...
    var topLabel  = document.getElementById('topLabel').value.trim() || 'On the menu';
    ...
    if (!brandName) return setStatus('error', 'Please enter a brand name.');
    ...
    setStatus('info', '⏳ Uploading files and creating your GitHub repo… this takes about 30 seconds.');
        ...
        if (data.error) {
          ...
          setStatus('success',
            ...
            '<div class="link-box" style="margin-top:12px">' +
              '<div style="font-size:10px;color:var(--muted);margin-bottom:4px">LIVE AR PAGE — ready in ~60 seconds</div>' +
              '<a href="' + data.liveUrl + '" target="_blank">' + data.liveUrl + '</a>' +
            '</div>' +
            '<div class="link-box">' +
              '<div style="font-size:10px;color:var(--muted);margin-bottom:4px">GITHUB REPO</div>' +
              '<a href="' + data.repoUrl + '" target="_blank">' + data.repoUrl + '</a>' +
            '</div>' +
            '<div class="tip">⏱ GitHub Pages takes ~60 seconds to activate. Bookmark the link above.</div>'
          );
        }
        btn.disabled = false;
...
function buildARPage(dishName, brandName, topLabel) {
  return `<!doctype html>
<html lang="en">
<head>
<meta charset="utf-8">
...
.top-label{display:flex;align-items:center;gap:10px}
...
.title-block{text-align:center;margin-top:16px}
...
.ar-btn{position:relative;width:min(66vw,270px);height:min(66vw,270px);border-radius:50%;background:none;border:none;cursor:pointer;-webkit-tap-highlight-color:transparent;display:flex;align-items:center;justify-content:center}
...
.steps{display:flex;align-items:flex-start;gap:8px;margin-top:32px}
...
.compat{margin-top:20px;display:flex;align-items:center;gap:12px;font-size:10.5px;color:var(--muted)}
  ...
  <div class="center-wrap">
    ...
    <button class="ar-btn" id="arBtn">
      ...
      <div class="face">
        <div class="rim"></div>
        <div class="brand">
          <div class="brand-name">On your<br>table</div>
