...
const { buildSocialRow, SOCIAL_CSS, cleanSocials } = require('./social');
...
app.post('/api/restaurants', (req, res) => {
  ...
  if (!restaurants[slug]) {
    restaurants[slug] = { name: name.trim(), branches: {}, socials: {} };
  }
...
app.post('/publish', upload.fields([
  ...
  try {
    await pushFile(folderPath + '/index.html', Buffer.from(buildARPage(dishName, brandName, topLabel, logoFileName, theme, restaurant.socials), 'utf8'));
...
body{background:
  radial-gradient(1400px 700px at 15% -10%, rgba(200,135,58,0.10), transparent 55%),
  radial-gradient(1000px 600px at 100% 110%, rgba(200,135,58,0.06), transparent 55%),
  var(--bg);
  background-attachment:fixed}

/* ── APP SHELL: sidebar + workspace ──────────────────────────────── */
body.app{display:grid;grid-template-columns:260px 1fr;grid-template-rows:100vh;padding:0;overflow:hidden}
.sidebar{
  background:linear-gradient(180deg, rgba(26,24,18,0.92), rgba(17,16,9,0.92));
  border-right:1px solid var(--border);
  display:flex;flex-direction:column;padding:26px 20px;height:100vh;position:relative}
.sidebar::after{content:"";position:absolute;top:0;right:0;width:1px;height:100%;
  background:linear-gradient(180deg,transparent,rgba(200,135,58,.35),transparent)}
.brand{display:flex;align-items:baseline;gap:2px;margin-bottom:4px}
.brand-mark{font-family:'Cormorant Garamond',serif;font-size:27px;font-weight:600;letter-spacing:.04em;color:var(--cream)}
.brand-mark span{color:var(--amber)}
.brand-tag{font-size:9.5px;letter-spacing:.32em;text-transform:uppercase;color:var(--muted);margin-bottom:34px;padding-left:2px}
.nav{display:flex;flex-direction:column;gap:5px}
.nav-item{display:flex;align-items:center;gap:12px;padding:12px 14px;border-radius:10px;
  background:none;border:1px solid transparent;color:var(--muted);font-family:inherit;font-size:14px;font-weight:500;
  cursor:pointer;text-align:left;width:100%;transition:all .16s}
.nav-item:hover{background:rgba(200,135,58,.07);color:var(--cream)}
.nav-item.active{background:rgba(200,135,58,.13);border-color:rgba(200,135,58,.32);color:var(--amber)}
.nav-item .ico{font-size:16px;width:18px;text-align:center}
.sidebar-foot{margin-top:auto;padding-top:20px;border-top:1px solid var(--border)}
.status-pill{display:flex;align-items:center;gap:8px;font-size:11px;color:var(--muted);margin-bottom:14px}
.status-dot{width:7px;height:7px;border-radius:50%;background:var(--green);box-shadow:0 0 0 3px rgba(76,175,125,.15);animation:livePulse 2.4s ease-in-out infinite}
@keyframes livePulse{0%,100%{opacity:1}50%{opacity:.45}}
.logout-btn{display:block;width:100%;text-align:center;background:none;border:1px solid var(--border);color:var(--muted);
  font-size:12px;padding:9px;border-radius:8px;cursor:pointer;font-family:inherit;text-decoration:none;transition:all .16s}
.logout-btn:hover{border-color:rgba(200,135,58,.5);color:var(--cream)}

/* workspace */
.workspace{height:100vh;overflow-y:auto;padding:0}
.topbar{position:sticky;top:0;z-index:20;display:flex;align-items:center;justify-content:space-between;
  padding:22px 40px;background:rgba(17,16,9,0.82);backdrop-filter:blur(14px);border-bottom:1px solid var(--border)}
.topbar-title{font-family:'Cormorant Garamond',serif;font-size:30px;font-weight:600;line-height:1}
.topbar-sub{font-size:12.5px;color:var(--muted);margin-top:5px}
.topbar-meta{display:flex;gap:26px;align-items:center}
.metric{text-align:right}
.metric-num{font-family:'DM Sans',sans-serif;font-size:22px;font-weight:600;color:var(--amber);font-variant-numeric:tabular-nums;line-height:1}
.metric-lbl{font-size:9.5px;letter-spacing:.16em;text-transform:uppercase;color:var(--muted);margin-top:4px}
.workspace-inner{padding:34px 40px 90px;max-width:1180px}

/* section visibility driven by existing showTab() */
.section{display:none}
.section.active{display:block;animation:fadeUp .4s ease}
@keyframes fadeUp{from{opacity:0;transform:translateY(8px)}to{opacity:1;transform:none}}

/* the old .card becomes a flat workspace panel; keep class for compatibility */
.card{width:100%;max-width:100%;background:transparent;border:none;border-radius:0;padding:0;box-shadow:none}
/* legacy header/tabs hidden — replaced by sidebar */
header.legacy,.tabs{display:none}

@media(max-width:820px){
  body.app{grid-template-columns:1fr;grid-template-rows:auto 1fr;overflow:auto}
  .sidebar{height:auto;flex-direction:row;flex-wrap:wrap;align-items:center;gap:12px;padding:16px 20px}
  .sidebar::after{display:none}
  .brand-tag{display:none}.brand{margin-bottom:0;margin-right:auto}
...
/* legacy header styles retained for login page */
...
/* TABS (legacy, hidden on app but kept for safety) */
...
` : `
<aside class="sidebar">
  <div class="brand"><span class="brand-mark">Ser<span>vision</span></span></div>
  <div class="brand-tag">AR Publishing Console</div>
  <nav class="nav">
    <button class="nav-item active" id="tab-btn-publish" onclick="showTab('publish')"><span class="ico">◆</span><span class="lbl">Publish Dish</span></button>
    <button class="nav-item" id="tab-btn-restaurants" onclick="showTab('restaurants')"><span class="ico">▦</span><span class="lbl">Restaurants</span></button>
  </nav>
  <div class="sidebar-foot">
    <div class="status-pill"><span class="status-dot"></span> System online · ar.servision.ca</div>
...
<main class="workspace">
  <div class="topbar">
    <div>
      <div class="topbar-title" id="ws-title">Publish Dish</div>
      <div class="topbar-sub" id="ws-sub">Turn a 3D model into a live AR experience</div>
    </div>
...
function buildARPage(dishName, brandName, topLabel, logoFileName, theme, socials) {
  const THEMES = {
    'dark-elegant': {
